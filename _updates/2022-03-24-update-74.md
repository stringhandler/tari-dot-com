...
Every base node has it's own public key and address, which is either an IP version 4 address or Tor hidden service onion address. There is no cost to running a node with a Tor address, so a malicious user can create
many different nodes very quickly, quicker than an honest base node could ban them. IP addresses are not as easy to acquire and there's a limited amount of them. In principle this makes  

In future, Tari may add minimum restrictions on how many IP4 peers it keeps, but until then this warning will allow the user to switch to a TCP only connection.
...
To enable metric, run the base node with the following:

`tari_base_node --metrics-bind-addr 127.0.0.1:1234`

Where `127.0.0.1:1234` is your [Prometheus]
