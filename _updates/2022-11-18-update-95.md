...
In v0.39.0, the SHA3 proof of work algorithm is updated from doing 2 rounds to 3 rounds of hashing. Two new output types, ValidatorNodeRegistration and CodeTemplateRegistration have been merged into development. In future, these will be used by the Tari Digital Assets Network, but are currently not allowed on Esmeralda.

From v0.40.0 onwards, EmojiIds now include the network as well as the checksum. This mechanism prevents sending to a different testnet and will be especially
important once we have a Mainnet. There's no visual difference between EmojiId addresses on different networks, but the wallets are able to confirm the network before sending funds. For interactive payments, this wouldn't be a problem, since the two wallets are not be able to communicate across the networks, but one-sided payments would end up in funds being lost.
